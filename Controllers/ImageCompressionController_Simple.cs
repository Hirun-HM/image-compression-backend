...
public class ImageCompressionController : ControllerBase
{
        ...
        [FromForm] bool enableAnalysis = true)
    {
        try
        {
            ...
            // Perform compression
            var result = await _compressionService.CompressImageAsync(imageData, image.FileName, options);
            
            // Set download URL
            result.DownloadUrl = $"/api/compression/download/{result.Id}";
    ...
    public async Task<IActionResult> AnalyzeImage(IFormFile image)
    {
        try
        {
            ...
            var analysis = await _compressionService.AnalyzeImageAsync(imageData, image.FileName);
